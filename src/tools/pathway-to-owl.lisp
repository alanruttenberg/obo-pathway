(defvar *opath-term-to-uri* (make-hash-table :test 'equalp))

(defmethod handle-uri :around ((o parsed-handle))
  (or (call-next-method) (setf (handle-uri o) (compute-handle-uri o))))

(defun uri-quoted-handle (s)
  (replace-all s "([<>\" {}|\\`^])" 
	       (lambda(c) (format nil "%~x" (char-code (char c 0)))) 1))
...
(defun legacy-uri (prefix id handle)
  (cond ((#"matches" id "^\\d+$")
	...
	((or (equal id "submitted") (equal id "tosubmit"))
	 (make-uri (make-uri (format nil "http://purl.org/obo/owl/~a#submitted_~a" prefix (uri-quoted-handle (string-downcase handle))))))
	((or (equal id "?") )
	 (make-uri (make-uri (format nil "http://purl.org/obo/owl/~a#maybe_~a" prefix (uri-quoted-handle (string-downcase handle))))))
...
(defun obolibrary-uri (prefix id handle)
  (cond ((#"matches" id "^\\d+$")
	...
	((or (equal id "submitted") (equal id "tosubmit"))
	 (make-uri (make-uri (format nil "http://purl.obolibrary.org/obo/~a_submitted_~a" prefix (string-downcase (uri-quoted-handle handle))))))
	((or (equal id "?") )
	 (make-uri (make-uri (format nil "http://purl.obolibrary.org/obo/~a_maybe_~a" prefix (string-downcase (uri-quoted-handle handle))))))
	(t (error "Don't know how to make uri for ~a:~a (~a)" prefix id handle))))
  ...
  (let ((handle (handle o))
    ...
    (flet ((compute-uri (base prefix id handle)
	     (cond ((#"matches" id "^\\d+$")
		    (make-uri (format nil "http://purl.obolibrary.org/obo/~a_~a" prefix id)))
		   ((or (equal id "submitted") (equal id "tosubmit"))
		    (make-uri (format nil "http://purl.obolibrary.org/obo/~a_submitted_~a" prefix (string-downcase (uri-quoted-handle handle)))))
		   ((or (equal id "?"))
		    (make-uri (format nil "http://purl.obolibrary.org/obo/~a_maybe_~a" prefix (string-downcase (uri-quoted-handle handle)))))
	...
	(destructuring-bind (prefix id) (or (car (all-matches id "(.*):(.*)" 1 2))
	  ...
	  (cond ((equal prefix "GO")
		 (assert (or (#"matches" id "^\\d+") (equal id "submitted")  (equal id "tosubmit") (equal id "?")) (id) "GO id GO:~a malformed" id)
		...
		(t (error "compute-handle-uri prefix ~a in ~a" prefix o))))))))
    ...
    (cond ((member ont-name '("CHEBI" "PFAM" "PATO" "SO") :test 'equal)
	  ...
	  ((equal ont-name "PRO")
	   (let ((type (handle-kind o)))
	     (cond ((search "complex" type :test #'char-equal)
		   ...
		   ((or (equalp type "post transcriptional modification") (equalp type "post transcriptional modifiction"))
	  ...
	  ((equal ont-name "GO")
	   (let ((type (handle-kind o)))
	     (cond ((member type '("function" "molecular function") :test 'equalp)
		   ...
		   ((member type '("CC" "GO CC" "cellular component" "celullar component" "protein complex") :test 'equalp)
...
(defmethod write-external.owl ((book opath-pathway-book) &key (destdir "opath:immunology;proto;"))
  (let ((axioms 
    ...
    (let ((ont-uri (make-uri nil (format nil "obo:opath/dev/~a-pathway-external.owl" (#"replaceAll" (pathway-name book) "\\s+" "-")))))
      (with-ontology external (:about ont-uri :base !obo: :collecting t)
	  ((as `(imports ,!obo:iao/ontology-metadata.owl))
	   (as axioms))
...
(defmethod write-external-derived.owl ((book opath-pathway-book))
  (let ((name (#"replaceAll" (pathway-name book) "\\s+" "-")))
    (create-external-derived :kb (load-ontology (format nil "~a~a-pathway-external.owl" (truename "opath:immunology;proto;") name))
			     :output-path (format nil "~a~a~a" (namestring (truename "opath:immunology;proto;")) name "-pathway-external-derived.owl" )
			     :templates-path "opath:tools;immunology-external-templates.txt"
			     :ontology-uri (format nil "http://purl.obolibrary.org/obo/opath/dev/~a-pathway-external-derived.owl" name))))

'(create-external-derived :kb (load-kb-jena "/Users/alanr/repos/infectious-disease-ontology/trunk/src/ontology/immunology/proto/pathway-external.owl") :output-path "/Users/alanr/repos/infectious-disease-ontology/trunk/src/ontology/immunology/proto/pathway-external-derived.owl" :templates-path "~/repos/infectious-disease-ontology/trunk/src/tools/immunology-external-templates.txt" :ontology-uri (uri-full !obo:opath/dev/pathway-external-derived.owl))

(defmethod owl-axioms-for-processes ((book opath-pathway-book))
...
(def-uri-alias "substrate-disposition" !obi:OPATH_0000001)
(def-uri-alias "product-disposition" !obi:OPATH_0000002)
(def-uri-alias "has-n-inputs" !obi:OPATH_0000007)
(def-uri-alias "has-n-outputs" !obi:OPATH_0000008)
...
(def-uri-alias "has-granular-process-part" !<http://purl.obolibrary.org/obo/BFO_0000072>)

(defmethod write-pathway.owl ((book opath-pathway-book) &key (destdir "opath:immunology;proto;"))
  (let ((ont-uri (make-uri nil (format nil "obo:opath/dev/~a-pathway.owl" (#"replaceAll" (pathway-name book) "\\s+" "-")))))
    (with-ontology spreadsheet (:about ont-uri :eval t)
	`((imports ,(make-uri nil (format nil "obo:opath/dev/~a-pathway-external.owl" (#"replaceAll" (pathway-name book) "\\s+" "-"))))
	  (imports !obo:opath/dev/pathway-defs.owl)
	  (imports ,(make-uri nil (format nil "obo:opath/dev/~a-pathway-external-derived.owl" (#"replaceAll" (pathway-name book) "\\s+" "-"))))
	  ...
	  (declaration (object-property !has-output))
...
(defun fresh-immunology-uri ()
  (make-uri (format nil "http://purl.obolibrary.org/obo/OPATH_~7,'0d" (incf *immunology-uri-id-counter*))))
    ...
    (if  (or (parse-errors p)
	 ...
	 (let ((uri (process-uri p)))
	   `((declaration (class ,uri))
	     ...
	     (annotation-assertion !has-n-outputs ,uri (:literal ,(length (process-products p)) !xsd:integer))
	     (annotation-assertion !has-n-inputs ,uri (:literal ,(length (process-substrates p)) !xsd:integer))
	     ,@(process-curated-realizations-axioms p)
	     ,@(process-part-located-in-axiom (setq @ p))
	     ,(only-has-protein-participants-axiom p)
	     ,(spreadsheet-source-editor-note p)
	     (sub-class-of ,uri !biological-process))))))

(defmethod owl-axioms-for-pathway ((book opath-pathway-book))
  (let ((uri (fresh-immunology-uri)))
   `(
      (declaration (class ,uri))
      (annotation-assertion ,!obo:IAO_0000116 ,uri ,(format nil "This is the pathway described by the workbook ~a"
							   (pathname-name (book-path book))))
      (annotation-assertion !rdfs:label ,uri ,(pathway-name book))
      (subclassof ,uri !biological-process)
      ,@(loop for bl in (blocks-of-type book 'parsed-process-block)
	   append
	     (loop for p in (parsed-rows bl)
		unless (or (parse-errors p)
			      (not (all-participant-handles-defined?  p)))
		    collect
		    `(subclassof ,uri (object-some-values-from !has-granular-process-part ,(process-uri p))))))))
