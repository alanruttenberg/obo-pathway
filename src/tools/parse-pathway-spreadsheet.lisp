...
(defclass opath-pathway-book (parsed-book)
  ((sheet-type :initform 'opath-pathway-sheet)
   (pathway-name :accessor pathway-name :initarg :pathway-name)
   (uri-prefix :accessor uri-prefix :initarg :uri-prefix :initform "IPATH_")
   (uri-index :accessor uri-index :initarg :uri-index :initform 0)
   ))

(defmethod lookup-handle ((b opath-pathway-book) name)
  (let ((results 
...
(defmethod where-is-handle ((b opath-pathway-book) name)
  (loop for sheet in (parsed-sheets b)
     for found = (lookup-handle sheet name)
     when found collect sheet))

(defmethod parse-book ((b opath-pathway-book) &key (handles? t))
...
(defmethod after-all-sheets-parsed ((b opath-pathway-book))
  (classify-handles b)
...
(defmethod block-types ((book opath-pathway-book))
  (loop for class in (remove-duplicates (mop::class-direct-subclasses (find-class 'opath-pathway-block)) :key 'class-name)
       collect (list* (class-name class) (class-slot-value class 'block-headers))))

(defmethod blocks-of-type  ((book opath-pathway-book) type)
  (remove type (parsed-blocks book) :test-not 'eq :key 'type-of))

(defmethod foreach-row-in-block-type ((book opath-pathway-book) type fn)
...
(defclass opath-pathway-sheet (parsed-sheet)
...
(defmethod after-all-sheets-parsed ((s opath-pathway-sheet))
  (handle-lookup-sheets s))

(defmethod handle-lookup-sheets ((s opath-pathway-sheet))
...
(defmethod clear-handles ((s opath-pathway-sheet))
  (setf (lookup-table s) (make-hash-table :test 'equalp)))

(defmethod add-handle ((s opath-pathway-sheet) name object)
  (if (not (stringp name))
      (push (format nil "handle isn't a string: '~a':~a,~a" name s object)  (parse-errors object))
      (if (not (#"matches" name "^[^<>\" {}|\\^`]+$"))
	  (push (format nil "handle uses an invalid character: '~a':~a,~a" name s object)  (parse-errors object))
...
(defmethod lookup-handle ((s opath-pathway-sheet) name)
...
(defmethod classify-handles ((book opath-pathway-book))
...
(defmethod verify-process-handles ((book opath-pathway-book))
  (loop for block in (blocks-of-type book 'parsed-process-block)
     append
       (map nil 'verify-process-handles (parsed-rows block))))

(defmethod parse-realizations ((book opath-pathway-book))
  (loop for block in (blocks-of-type book 'parsed-process-block)
     append
       (map nil 'parse-realizations (parsed-rows block))))

(defmethod parse-process-part-of ((book opath-pathway-book))
  (loop for block in (blocks-of-type book 'parsed-process-block)
     append
       (map nil 'parse-part-of (parsed-rows block))))

(defmethod parse-bindings ((book opath-pathway-book))
  (loop for block in (blocks-of-type book 'parsed-process-block)
     append
...
(defclass opath-pathway-block (parsed-block))

(defmethod block-headers ((s symbol))
  (block-headers (mop::class-prototype (find-class s))))

(defclass parsed-handle-block (opath-pathway-block )
...
(defclass parsed-process-block (opath-pathway-block)
...
(defclass parsed-complex-block (opath-pathway-block)
...
(defclass parsed-evidence-block (opath-pathway-block)
...
(defclass parsed-sheet-info-block (opath-pathway-block)
...
(defclass opath-pathway-cells (parsed-cells))
...
(defclass parsed-handle (opath-pathway-cells)
...
(defclass parsed-process (opath-pathway-cells)
...
(defclass parsed-complex (opath-pathway-cells)
  ((handle :accessor handle :initarg :handle :initform nil)
...
(defclass parsed-evidence (opath-pathway-cells)
  )
...
(defun check-pathway-spreadsheet-syntax (&key (path "ido:immunology;ido-s4lps-tlr4.xlsx") (name "tlr4"))
  (let* ((book (make-instance 'opath-pathway-book :book-path path :pathway-name name)))
